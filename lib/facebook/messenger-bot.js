...
_.extend(Bot.prototype, {
    ...
    setPersistentMenu: function (items, cb) {
        request({
            method: "POST",
    ...
    expressMiddleware: function (express) {
        ...
        express.get("/redbot/facebook/test" + bot.webhookURL, function (req, res) {
            bot._handleMessage({
                object: "page",
                entry: [
                    {
        ...
        express.get("/redbot/facebook/_status" + bot.webhookURL, function (req, res) {
            res.send({ status: "ok" });
        });

        express.get("/redbot/facebook" + bot.webhookURL, function (req, res) {
            bot._verify(req, res);
        });

        express.post("/redbot/facebook" + bot.webhookURL, function (req, res) {
